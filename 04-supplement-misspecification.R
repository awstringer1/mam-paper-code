...
## random effects drawn from mixture of 2 gaussians
sigma0 <- 1
sigma1 <- 3
nn <- 5000
mix <- rbinom(nn,1,0.5)
yy0 <- rnorm(nn,0,sigma0)
yy1 <- rnorm(nn,0,sigma1)
yy <- (1-mix)*yy0+mix*yy1 ## mixture of 2 normals
...
get_data_misp <- function(conditional,x1,x2,x3,K,Nk){
  ...
  u0 <- rnorm(K,0,sigma0)
  u1 <- rnorm(K,0,sigma1)
    V <- as.data.frame((1-mix)*u0+mix*u1)
  ...
  dat$fx1 <- f1(x1)
  dat$fx2 <- f2(x2)
  dat$mix <- rep(mix,each=Nk)
  
  Ui <- dat[,c("intercepts")]

  if(conditional==FALSE){
    ## approximation from Hedeker
    vars <- (1-dat$mix)*sigma0^2+dat$mix*sigma1^2+##SSmat[1,1]+ ##
    ...
    ## generate from GAMM
    dat <- within(dat,{y <- rbinom(K*Nk,1,mam::ilogit(fx1 + fx2 + beta3*x3 + Ui))})
...
do_simulation_misp <- function(K,Nk,conditional,iter=0,verbose=TRUE) {

  ## generate data
  x1 <- round(runif(K*Nk,-1,1),3)
  x3 <- round(runif(K*Nk,-1,1),3)
  x2 <- round(runif(K*Nk,-1,1),3)
  dat <- get_data_misp(conditional,x1,x2,x3,K,Nk)
  gridlen=100
  ...
  ## using our own GAMM implementation since gamm4 is the bottleneck (and performs the same as ours in main simulations)
  # # (vi) GAMM (conditional)
  ...
  fitteddat <- as_tibble(dat2pred) %>%
    mutate(
      ...
      K=K,Nk=Nk,conditional,iter=iter,
      ## mam
  ...
  # mamtheta <-c(sqrt(themam$conditional$theta[1,1]))
  # gammtheta <- c(sqrt(VarCorr(gamm$mer)$id[1,1]))
  ...
  return(list(fitteddat, preddat))
}

## Simulations ##
B <- 2000

simstodo <- expand.grid(
  K = c(100,200),
  Nk = c(10,20),
...
dosim <- function(lst) {
  ...
  sim <- tryCatch(do_simulation_misp(K=lst[1],Nk=lst[2],conditional=lst[3],iter=lst[4]),error=function(e) e)
  sim$sim <- lst[4]
  sim
}
...
save(sims2,file = file.path(savepath,paste0("simsraw-misp-",suffix,".RData")))
# save(simlist2,file = file.path(savepath,paste0("sims-misp-",suffix,".RData")))
...
covr_x1 <- simfitted %>%
  filter(x2==0, x3==0) %>%
  group_by(x1,K,Nk,conditional) %>%
  summarize(
    mammargcovr=mean(mammargcovrx1),
    mamcondcovr=mean(mamcondcovrx1)#,
    # gammcondcovr=mean(gammcondcovrx1)
  )

covr_x2 <- simfitted %>%
  filter(x1==0, x3==0) %>%
  group_by(x2,K,Nk,conditional) %>%
  summarize(
    mammargcovr=mean(mammargcovrx2),
    mamcondcovr=mean(mamcondcovrx2),
...
sesd_x1 <- simfitted %>%
  ...
  group_by(x1,K,Nk,conditional) %>%
  summarize(
    sesd_mammarg = (mean(mammargse)/sd(mammargfitted))-1,
    sesd_mamcond = (mean(mamcondse)/sd(mamcondfitted))-1
  )
...
estimationtable_x1 <- simfitted %>%
  filter(x2==0,x3==0) %>%
  group_by(K,Nk,x1,conditional) %>%
  summarize(
    bias_x1_mammarg = mean(mammargbiasx1),
    covr_x1_mammarg = mean(mammargcovrx1),
    sesd_x1_mammarg = (mean(mammargse)/sd(mammargfitted)),
    bias_x1_mamcond = mean(mamcondbiasx1),
    covr_x1_mamcond = mean(mamcondcovrx1),
    sesd_x1_mamcond = (mean(mamcondse)/sd(mamcondfitted))
  )%>%
  group_by(K,Nk,conditional) %>% ## second grouping because now we collapse over x1 (now that we have ratios of SE to SD)
  summarize_all(mean)
  
estimationtable_x2 <- simfitted %>%
  filter(x1==0,x3==0) %>%
  group_by(K,Nk,x2,conditional) %>%
  summarize(
    bias_x2_mammarg = mean(mammargbiasx2),
    covr_x2_mammarg = mean(mammargcovrx2),
    sesd_x2_mammarg = (mean(mammargse)/sd(mammargfitted)),
    bias_x2_mamcond = mean(mamcondbiasx2),
    covr_x2_mamcond = mean(mamcondcovrx2),
    sesd_x2_mamcond = (mean(mamcondse)/sd(mamcondfitted))
  ) %>%
  group_by(K,Nk,conditional) %>% ## second grouping because now we collapse over x2 (now that we have ratios of SE to SD)
  summarize_all(mean)


predinttable <- simpred %>%
  group_by(K,Nk,conditional) %>%
  summarize(
...
write_csv(resultstable,file = file.path(plotpath,"numeric-summaries-misp.csv"))
...
resultstable %>%
  ...
  mutate(across(contains('sesd'),~((.x-1)))) %>%
  dplyr::select(-contains(c("slope","cor"))) %>%
  dplyr::select(contains(c("K","Nk","conditional",'mamcond',"RMSEP"))) %>% 
  dplyr::select(!contains(c('gamm'))) %>%
  knitr::kable(
    digits = 2,
    format = 'latex' # markdown, not latex, easier copying
